...
from wolframclient.evaluation import WolframLanguageAsyncSession, WolframKernelPool
...
class TestCoroutineSession(BaseTestCase):
    ...
    async def test_eval_wlsymbol(self):
        start = time.perf_counter()
        task = asyncio.ensure_task(
            self.async_session.evaluate(wl.CompoundExpression(wl.Pause(.1), wl.Range(2)))
        )
        timer = time.perf_counter() - start
        self.assertTrue(timer < .1)
        res = await task
...
class TestKernelPool(BaseTestCase):
    ...
    @classmethod
    def setupKernelSession(cls):
        cls.pool = WolframKernelPool(cls.KERNEL_PATH, kernel_loglevel=logging.INFO,
    ...
    async def test_eval_wlsymbol(self):
        tasks = [
            asyncio.ensure_task(self.pool.evaluate(
                wl.FromLetterNumber(i)))
            for i in range(1,11)
        ]
        res = await asyncio.gather(*tasks)
    ...
    async def test_eval_wxf(self):
        tasks = [
            asyncio.ensure_task(self.pool.evaluate_wxf(
                'FromLetterNumber[%i]' % i))
            for i in range(1,11)
        ]
        res = await asyncio.gather(*tasks)
